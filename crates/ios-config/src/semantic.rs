...
// Pass 2: walk the RawTree and populate the IR.
// Unrecognized nodes go into UnknownBlock rather than being silently dropped.
...
impl SemanticParser {
    pub fn analyze(&self, tree: &RawTree) -> NetworkConfig {
        ...
        // First pass: collect named NAT pools so that pool references in
        // `ip nat inside source list X pool NAME` can be resolved regardless
    ...
    /// Collect all `ip nat pool` declarations into a name-keyed map.
    /// Done in a separate pass so pool references can be resolved regardless
    /// of whether the pool declaration appears before or after the `ip nat inside source` line.
    ...
    fn dispatch_global(
        ...
        match node.keyword().to_lowercase().as_str() {
            ...
            // `enable secret/password` — Cisco enable-mode credential.
            // Stored in platform_specific as it has no direct cross-platform equivalent.
            "enable" => {
            ...
            // End-of-config marker — ignore.
            "end" => {}
            // IOS platform-specific commands — store separately, not in unknown_blocks.
    ...
    fn parse_interface_child(
        ...
        match tokens[0] {
            ...
            "no" if tokens.len() >= 2 => match tokens[1] {
                ...
                // `no ip address` — clear IP; ignore silently.
                "ip" if tokens.get(2) == Some(&"address") => {} // `no ip address` — ignore
                _ => {} // other `no` commands — ignore
            },
            ...
            "storm-control" => {
                // storm-control {broadcast|multicast|unicast} level <pct>
            ...
            "spanning-tree" => {
                // spanning-tree {portfast|bpduguard|bpdufilter|guard root}
                match tokens.get(1) {
    ...
    fn parse_ip_address(&self, tokens: &[&str]) -> Option<IpNet> {
        ...
        // Dotted-mask notation: "192.168.1.1 255.255.255.0"
        if tokens.len() >= 2 {
        ...
        match tokens.first() {
            ...
            Some(&"authentication") => {
                if tokens.get(1) == Some(&"message-digest") {
                    // Key is added by a separate message-digest-key command.
                    let ospf = iface.ospf.get_or_insert_with(|| default_iface_ospf());
    ...
    fn parse_hsrp(&self, tokens: &[&str]) -> Option<HsrpGroup> {
        ...
        match tokens.get(1) {
            ...
            Some(&"timers") => {
                ...
                // Simplified: seconds-only parsing (msec variants not yet handled).
    ...
    fn handle_global_ip(
        ...
        match tokens[1] {
            ...
            "default-gateway" => {
                // `ip default-gateway` — treated as a 0.0.0.0/0 static route.
            ...
            "http" => {
                // `ip http [secure-]server` — platform-specific; store and move on.
                cfg.platform_specific.push(UnknownBlock {
                    line: node.line_num,
                    context: "global".to_string(),
                    raw: node.full().to_string(),
    ...
    fn parse_ospf_process(&self, pid: u32, node: &RawNode, cfg: &mut NetworkConfig) -> OspfProcess {
        ...
        // Area map keyed by area string.
        // Vec rather than HashMap so iteration order matches the source config
        // (OSPF configs rarely have more than a handful of areas, so O(n) lookup is fine).
    ...
    fn parse_bgp_neighbor_cmd(
        ...
        // Determine whether the address is an IP or a peer-group name.
        let addr = if let Ok(ip) = addr_str.parse::<IpAddr>() {
            BgpNeighborAddr::Ip(ip)
        } else {
            // Not an IP address — treat as peer-group name.
            BgpNeighborAddr::PeerGroup(addr_str.to_string())
        };

        // `neighbor <name> peer-group` (no trailing name) — declares a peer-group.
        if tokens.get(1) == Some(&"peer-group") && tokens.len() == 2 {
            // `neighbor <name> peer-group` — peer-group declaration.
    ...
    fn apply_bgp_neighbor_attr(
        ...
        match tokens.get(1) {
            ...
            Some(&"peer-group") => {
                // `neighbor <ip> peer-group <name>` — assign neighbor to a peer-group.
            ...
            match ct.first() {
                ...
                Some(&"neighbor") => {
                    ...
                    match ct.get(2) {
                        ...
                        _ => {
                            // Other per-address-family neighbor attributes.
                            // apply_bgp_neighbor_attr expects [addr, attr, ...]
    ...
    fn handle_acl_global(&self, node: &RawNode, cfg: &mut NetworkConfig) {
        // Two forms:
        //   ip access-list [standard|extended] <name>  — named ACL (children = entries)
        //   access-list <number> permit/deny …         — numbered ACL (single line)

        let tokens: Vec<&str> = node.text.split_whitespace().collect();

        if tokens[0] == "access-list" {
            // Numbered ACL — single inline line.
    ...
    fn parse_numbered_acl_line(&self, node: &RawNode, cfg: &mut NetworkConfig) {
        ...
        // Find or create the ACL entry for this number.
    ...
    fn parse_acl_entry(&self, tokens: &[&str], default_seq: u32) -> Option<AclEntry> {
        ...
        if tokens.get(pos) == Some(&"remark") {
            return Some(AclEntry {
                sequence,
                action: AclAction::Permit, // irrelevant for remark entries
                protocol: None,
                src: AclMatch::Any,
                dst: None,
                src_port: None,
        ...
        // Protocol field (extended ACLs only).
        ...
        // Destination match (extended ACLs only).
    ...
    fn parse_acl_match(&self, tokens: &[&str]) -> (AclMatch, Option<AclPort>, usize) {
        ...
        match tokens[0] {
            "any" => {
                // Port qualifier may follow "any" (e.g. "permit tcp any eq 80 host X").
            ...
            "host" => {
                ...
                // Also parse any port qualifier that follows the host address.
            ...
            addr_str => {
                ...
                if let Some(wc_str) = tokens.get(1) {
                    if let Ok(wc) = wc_str.parse::<IpAddr>() {
                        ...
                        // Port qualifier may follow the wildcard mask.
                        let (port, pc) = self.parse_port(&tokens[2..]);
                        return (m, port, 2 + pc);
                    }
                }
    ...
    fn handle_nat(
        ...
        // Token layout: [0]=ip [1]=nat [2]=inside/outside/pool [3]=source/...
        let tokens: Vec<&str> = node.text.split_whitespace().collect();
        ...
        match tokens.get(3) {
                            ...
                            match tokens.get(pool_pos + 1) {
                                Some(pool_name) => {
                                    match nat_pools.get(*pool_name) {
                                        // Pool found — use its declared address range.
                                        Some(found) => {
                                            let mut p = found.clone();
                                            p.overload = overload;
                                            (Some(p), false)
                                        }
                                        // Pool referenced but not declared in this config.
                                        None => {
                                            cfg.unknown_blocks.push(UnknownBlock {
                                                line: 0,
            ...
            // `ip nat pool` declaration — already collected by collect_nat_pools(); accept silently.
...
fn wildcard_to_prefix(addr: IpAddr, wildcard: IpAddr) -> IpNet {
    // Invert the wildcard mask to get the network mask.
    match (addr, wildcard) {
...
fn parse_vlan_list(s: &str) -> Vec<u16> {
    // Parses a Cisco VLAN range string like "10,20,30-40,50".
    let mut result = vec![];
    for part in s.split(',') {
        if part.contains('-') {
...
impl SemanticParser {
    ...
    pub fn handle_aaa_global(&self, node: &RawNode, cfg: &mut NetworkConfig) {
        let tokens: Vec<&str> = node.text.split_whitespace().collect();
        // Record that AAA new-model is enabled.
        ...
        // aaa authentication/authorization method lists — stored in platform_specific
        // as the syntax varies significantly across platforms.
    }
}
